...
public class GroupingDetailSpecialSummary implements RemoteAccess {
	...
	public static XSSFWorkbook getAllInforOfPartStruct(WTPart toppart,WTPart rootpart) throws WTException
	{
		...
		/*	if (toppart.getName().indexOf("明细表") <= 0)
			...
			alltagMap=sortMapByKey(alltagMap);
			...
			throws WTException
	{
		...
		while (keyset.hasNext())
		{
			...
			if (partType != null && (partType.equals(STANDARD_PART) || (partType.equals(BUY_PART) || partType.equals(MAKE_PART))))
			{
				...
				System.out.println("zyj--ts--qt 2:"+subpart.getName());
					// 获取已经记录的父件link
					List parentlist = (List) hmap4.get(subpart);
...
//						double sum = usageLink.getQuantity().getAmount();
					...
					LinkedHashMap strMap = new LinkedHashMap();
					sumMap = new HashMap();
					Iterator keys = partMap.keySet().iterator();
					while(keys.hasNext()){
						WTPart pt = (WTPart) keys.next();
						String topPartName = pt.getName();
						int topIndex = topPartName.indexOf(NAME_FLEX);
						String topCode = topPartName.substring(0, topIndex);// 代号
						objMap.put(pt, partMap.get(pt));
						strMap.put(topCode, pt);
					}
					System.out.println("zyj--ts--strMap1 before:"+strMap);
					strMap=sortMapByKey(strMap);
					System.out.println("zyj--ts--strMap1 after:"+strMap);
					Iterator key = strMap.keySet().iterator();
					while(key.hasNext()){
						String tag= key.next()+"";
						WTPart pt = (WTPart) strMap.get(tag); 
						System.out.println("zyj--ts--sheetT  PART:"+stList.get(n) +"  ==  "+pt.getName());
						IBAUtility iba = new IBAUtility(pt);
						String type = (String) iba.getIBAValue(IBA_PART_TYPE);
						List ls = (List) objMap.get(pt);
						WTPartUsageLink lk = null;
						IBAUtility ibaUtil =null;
						String bz = "";
						if(ls!=null && ls.size()>0){
							...
							System.out.println("zyj--ts--lk:"+lk);
							if(lk!=null){
								double sum = lk.getQuantity().getAmount();
								System.out.println("zyj--ts--sum:"+sum);
								sumMap.put(pt, Double.valueOf(sum));
								System.out.println("sumMap de sum："+pt.getName()+"  "+sumMap.get(pt));
								ibaUtil  = new IBAUtility(lk);
							}
						}
						if(ibaUtil!=null){
...
//		System.out.println("zyj--ts--allstandardPartMap2.size:"+allstandardPartMap.size()+"===allstandardPartMap2:"+allstandardPartMap);
		...
		while (keyset.hasNext())
		{
			...
			if (tempsum == null)
				row.getCell(8).setCellValue(1);// 第8列为总数量
			else
			...
			if (parentList != null)
			{
				...
				for (int i = 0; i < parentsize; i++)
				{
					...
					if (i != 0)
					{
						...
						row = sheetNew.getRow(nextRowNo);
						row.getCell(0).setCellValue(orderNo);// 第一列为序号
						row.getCell(2).setCellValue("");// 代号
						row.getCell(3).setCellValue("");// 名称
						row.getCell(8).setCellValue("");// 总数
					}
