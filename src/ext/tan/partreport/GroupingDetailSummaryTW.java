...
public class GroupingDetailSummaryTW implements RemoteAccess {

	private static String CLASSNAME = GroupingDetailSummaryTW.class.getName();
	...
	public static XSSFWorkbook writeToExcel(WTPart toppart, LinkedHashMap allstandardPartMap, HashMap sumMap,WTPart rootpart, WTPart detailpart) throws WTException
	{
		...
		try
		{
			...
			// 往模板sheet插入抬头的数据
			writeTitleToSheet(toppart, sheetT, pageSize, 1, 32,detailpart);
	...
	public static void writeDataToSheet(XSSFSheet sheetNew, int allcount, int pageSize, HashMap allstandardPartMap, HashMap sumMap,WTPart rootpart,WTPart toppart) throws WTException
	{
		...
		while (keyset.hasNext())
		{
			...
			if (subpartname.indexOf("#") <= 0)
			{
				...
				row.getCell(3).setCellValue(subpartname);// 第3列为代号
				row.getCell(4).setCellValue("");// 第4列为名称及规格
				HashMap subIbaht = ReportUtil.getObjectValues(subpart, attrList);
				...
				if (drawing == null)
					drawing = "";
				row.getCell(2).setCellValue(drawing);// 幅面
				...
				if (material == "" && spec == "")
				{
					row.getCell(6).setCellValue(materialmark);// 第5列为材料、规格、牌号
				}
				else if (materialmark == "")
				{
					row.getCell(6).setCellValue(material + spec);// 第5列为材料、规格、牌号
				}
				else
				{
					row.getCell(6).setCellValue(material + spec + "\r\n" + materialmark);// 第5列为材料、规格、牌号
				}
				Double tempsum = (Double) sumMap.get(subpart.getNumber());
				if (tempsum == null)
					row.getCell(9).setCellValue(0);// 第8列为总数量
				else
					row.getCell(9).setCellValue(tempsum.doubleValue());// 第8列为总数量
				...
				if (parentList != null)
				{
					...
					if(parentsize>0){
						...
						if(nextRowNo==2)
							...
							row.getCell(7).setCellValue(parenypartname.substring(0, indexp));// 第6列为父件物料名称#前面
							row.getCell(8).setCellValue(qt.getAmount());// 第7列为BOM结构中数量
							row.getCell(10).setCellValue(qt.getUnit().getDisplay().replaceAll("每", ""));// 第9列为BOM结构中的单位
							row.getCell(11).setCellValue((String) linkIbaht.get(IBANAME_LINK_BZ));// 第10列为BOM结构中的备注
							String Remark=(String) linkIbaht.get(IBANAME_LINK_BZ);
					
							if(Remark!=null&&Remark.equals("借")){
								Remark= "";
								row.getCell(2).setCellValue(Remark);
							}
			...
			row.getCell(4).setCellValue(nameAndType);// 第4列为名称及规格
			HashMap subIbaht = ReportUtil.getObjectValues(subpart, attrList);
			...
			if (parentList != null)
			{
				...
				if(parentsize>0){
					...
					if (code.indexOf("-") > 0)
					{
						...
						row.getCell(7).setCellValue(fZpStr);// 第6列为父件物料名称#前面
						row.getCell(8).setCellValue( (Double) sumMap.get(subpart.getNumber()));// 第7列为BOM结构中数量
						row.getCell(10).setCellValue("个");// 第9列为BOM结构中的单位
					}
					if(orderNo%PAGE_SIZE==0){
						System.out.println("zyj--ts==nextRowNo+9");
						nextRowNo = nextRowNo + 10;
	...
	{
		for (int i = beginRowNo + 2; i < beginRowNo + EACH_PAGE_SIZE-7; i++)
		{
			CellRangeAddress region0 = new CellRangeAddress(i, i, 4, 5);
