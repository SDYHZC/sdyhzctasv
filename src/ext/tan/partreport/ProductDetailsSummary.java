...
public class ProductDetailsSummary implements RemoteAccess {

	private static String CLASSNAME = ProductDetailsSummary.class.getName();
			...
			for(int i=0;i<lastrowNo;i++)
			{
				if(i%EACH_PAGE_SIZE==0&&i!=0)
				{
				   ...
				   //合并表头单元格
					...
					//拷贝第一页的信息
					ExcelUtil.copyTemplateInforToSheet1(wb,sheetT,i,i/EACH_PAGE_SIZE+1,EACH_PAGE_SIZE,11);
				}
			}
			//先写sheet中要填写的结构信息
	...
	public static void writeTitleToSheet(WTPart toppart, XSSFSheet sheetT,int totalPageNo,int currentPageNo,int targetBeginRowNo) 
	{
       ...
       // String productnamebefore=ReportUtil.getNameStr(partname,NAME_FLEX, true);
		...
		XSSFCell tempcell=tempRow.getCell(6);
	...
	public static void writeDataToSheet(XSSFSheet sheetNew ,int allcount,int pageSize,List datalist)  
	{
		...
		for(int i=0;i<datalist.size();i++)
		{
			...
			System.out.println("zyj--ts==numList:"+numList.size());
			...
			row.getCell(1).setCellValue(orderNo);//第一列为序号
			row.getCell(3).setCellValue(code);//第3列为代号
			row.getCell(5).setCellValue(nameAndType);//第4列为名称 
			row.getCell(8).setCellValue(count);//第8列为页数
			row.getCell(10).setCellValue(bz);//第910列为备注
			if(numList.size()%PAGE_SIZE==0){
				System.out.println("zyj--ts==nextRowNo+10");
				nextRowNo = nextRowNo + 10;
			}else{
				System.out.println("zyj--ts==nextRowNo:"+nextRowNo);
				nextRowNo = nextRowNo + 1;
			}
			System.out.println("zyj--ts==nextRowNo+1");
			orderNo=orderNo+1;
		}
	...
	{
		for(int i=beginRowNo;i<beginRowNo+EACH_PAGE_SIZE-7;i++)
		{
			CellRangeAddress region0=new CellRangeAddress(i,i,1,2); 
			CellRangeAddress region1=new CellRangeAddress(i,i,3,4); 
			CellRangeAddress region2=new CellRangeAddress(i,i,5,7); 
			CellRangeAddress region3=new CellRangeAddress(i,i,8,9); 
			CellRangeAddress region4=new CellRangeAddress(i,i,10,11); 
	...
	public static String getNumberStr1(String str)
	{
	   StringBuffer buf=new StringBuffer("");
	   int index=str.lastIndexOf("-");
	   String tempstr=str;
	...
	public static List sortData(List datalist,HashMap topMap)
	{
		...
		LinkedHashMap treemap1 = new LinkedHashMap();
		...
		for (int i = 0; i < datalist.size(); i++)
		{
			...
			String keycode1 = getNumberStr1(tempcode);
			//处理备注字符串
			if(!"".equals(beizhu) && !"null".equals(beizhu) && beizhu!=null && beizhu.contains("与") && beizhu.contains("同")){
				...
				if(specMap.containsKey(dhStr)){
					l.add(dhStr);
					String st = dhStr +(l.size()+1)+"";
					specMap.put(st, tempmap);
				}else{
					specMap.put(dhStr, tempmap);
				}
			}else{
				if(!"".equals(keycode)){
					if(treemap.containsKey(keycode)){
						l1.add(keycode);
						String st = keycode +(l1.size()+1)+"";
						treemap.put(st, tempmap);
					}else{
						treemap.put(keycode, tempmap);
					}
				}else{
					if(treemap1.containsKey(keycode1)){
						l2.add(keycode1);
						String st = keycode1 +(l2.size()+1)+"";
						treemap1.put(st, tempmap);
					}else{
						treemap1.put(keycode1, tempmap);
					}
				}
			}
		...
		while (it.hasNext())
		{
			...
			while(iterator.hasNext()){
				...
				if(dh.contains(tempcode)){
					newdatalist.add(specMap.get(dh));
				}
			}
		}
		Iterator it1 = treemap1.keySet().iterator();
		while (it1.hasNext())
		{
			HashMap tpMap1 = (HashMap)treemap1.get(it1.next());
			newdatalist.add(tpMap1);
		}
		return newdatalist;
	}
	/**
