...
import wt.epm.attributes.EPMIBAConstraintFactory;
import wt.epm.attributes.EPMIBAConstraintFactory.EditFileBasedAttributes;
...
public class IBAHelper
{
  ...
  private static final Object EDIT_IBAS = new EPMIBAConstraintFactory.EditFileBasedAttributes();

  public HashMap ibaContainer = null;

  public static Object convertStringToIBAValue(String strVal, String dataType, Locale locale, TimeZone timezone)
    throws WTException
  {
    Object obj = null;
  ...
  {
    AttributeDefDefaultView theAttrDef = 
      IBADefinitionObjectsFactory.newAttributeDefDefaultView(
      IBAModel.getIBADefinitionByHid(wcAttrName));
    if (theAttrDef == null) {
      return null;
    }

    Class attrValueClass = getAttrValueClass(theAttrDef);
    AbstractValueView theAttr = null;
    if (attrValueClass == RatioValueDefaultView.class) {
      ...
      setRatioValues(theAttr, convertedNumeratorValue, 
        convertedDenominatorValue);
    } else if ((attrValueClass == FloatValueDefaultView.class) || 
      (attrValueClass == UnitValueDefaultView.class)) {
      ...
      theAttr = (AbstractValueView)constructor.newInstance(parameters);
      if (attrValueClass == UnitValueDefaultView.class)
        setValue(theAttr, getDefaultUnitValue(theAttr, 
          (Double)convertedValue));
    }
    else if (attrValueClass == TimestampValueDefaultView.class) {
      Class valueType = attrValueClass.getDeclaredField("value")
        .getType();
      Object convertedValue = getConvertedValue(value, valueType);
      Class[] parameterTypes = { theAttrDef.getClass(), valueType };
  ...
  private static Class getAttrValueClass(AttributeDefDefaultView attrDef) {
    if ((attrDef instanceof StringDefView)) {
      return StringValueDefaultView.class;
    }
    if ((attrDef instanceof FloatDefView)) {
      return FloatValueDefaultView.class;
    }
    if ((attrDef instanceof IntegerDefView)) {
      return IntegerValueDefaultView.class;
    }
    if ((attrDef instanceof TimestampDefView)) {
      return TimestampValueDefaultView.class;
    }
    if ((attrDef instanceof BooleanDefView)) {
      return BooleanValueDefaultView.class;
    }
    if ((attrDef instanceof URLDefView)) {
      return URLValueDefaultView.class;
    }
    if ((attrDef instanceof UnitDefView)) {
      return UnitValueDefaultView.class;
    }
    if ((attrDef instanceof RatioDefView)) {
      return RatioValueDefaultView.class;
    }
    if ((attrDef instanceof ReferenceDefView)) {
      return ReferenceValueDefaultView.class;
    }
    return null;
  }
  ...
  private static Double getDefaultUnitValue(AbstractValueView attributeValueView, Double displayValue)
    throws IOException, UnitFormatException, IncompatibleUnitsException
  {
    ...
    String measurementSystem = 
      MeasurementSystemCache.getCurrentMeasurementSystem();
  ...
  static TypeInstance getIBAValuesInternal(Object obj, ArrayList ibaList, HashMap ibaMap) throws WTException
  {
    ...
    if ((obj instanceof IBAHolder)) {
      ...
      IdentifierFactory idFactory = (IdentifierFactory)
        DefaultServiceProvider.getService(
    ...
    try {
      PopulatedAttributeContainerFactory pacFactory = (PopulatedAttributeContainerFactory)
        DefaultServiceProvider.getService(PopulatedAttributeContainerFactory.class, 
        "virtual");
  ...
  private static WTException interpretConstraintViolationException(ConstraintException constraintexception, Locale locale)
    throws WTException
  {
    ...
    String s4 = null;
    Serializable serializable = constraintdata
    ...
    throws WTException
  {
    ...
    for (int i = 0; i < listIBAId.size(); i++) {
      AttributeTypeIdentifier ati = 
        (AttributeTypeIdentifier)listIBATypeId
        .get(i);
    ...
    DefaultConstraintValidator dac = 
      DefaultConstraintValidator.getInstance();
      ...
      for (int i = 0; i < ais.length; i++) {
        ...
        catch (ConstraintException ce)
        {
          if (ce
            ...
            if ((vmap != null) && (vmap.get(ais[i]) != null)) {
              if (((vmap
                .get(ais[i]) instanceof Comparable)) && 
                (((Comparable)ti
                .get(ais[i])).compareTo(vmap.get(ais[i])) == 0)) continue;
              if (vmap.get(ais[i]).equals(ti.get(ais[i])))
              {
                continue;
              }
            }
  ...
  private static void setRatioValues(AbstractValueView attributeValueView, Object numeratorValue, Object denominatorValue) throws WTException
  {
    try {
      ...
      Object[] numeratorMethodParameters = { 
        new Double(((Double)numeratorValue).doubleValue() / 
  ...
  private static boolean updateAttributes(DefaultAttributeContainer dac, String wcAttrName, String attrValue, Locale locale, AbstractValueView[] avv)
    throws Exception
  {
  ...
  {
    this.ibaContainer = new HashMap();
    if (holder != null)
      getIBAValuesLite(holder, this.ibaContainer);
  }

  public String getIBAValue(String ibaName) {
    return (String)this.ibaContainer.get(ibaName);
  }
}