    ...
    try {
      if ((obj instanceof WTChangeOrder2)) {
        ...
        while (qr.hasMoreElements()) {
          ...
          QueryResult qrs = RelatedChangesQueryCommands.getRelatedAffectingChangeNotices(changeable2);
    ...
    try {
      if ((obj instanceof WTChangeOrder2)) {
        ...
        while (qr.hasMoreElements()) {
          Persistable pobj = (Persistable)qr.nextElement();
          if ((!(pobj instanceof WTPart)) || 
            (!((WTPart)pobj).getViewName().equals("Manufacturing"))) continue;
          flag = true;
          break;
        }
      }
    ...
    try {
      if ((obj instanceof WTChangeOrder2)) {
        ...
        while (qr.hasMoreElements()) {
          Persistable pobj = (Persistable)qr.nextElement();
          if ((!(pobj instanceof WTPart)) || 
            (!((WTPart)pobj).getViewName().equals("Design"))) continue;
          flag = true;
          break;
        }
      }
    ...
    try
    {
      ...
      while (qr.hasMoreElements())
        try {
          ...
          //防止非默认ECA状态被设置
          if(ca.getName().contains(ecn.getNumber())) continue;
          State stateDist = State.toState(ECNState);
          LifeCycleHelper.service.setLifeCycleState(
            ca, stateDist);
        }
...
//        QueryResult qr = ChangeHelper2.service.getChangeablesAfter(ecn, true);
        //转换成使用变更活动的方式获取数据，以便过滤非默认变更活动
        QueryResult qr1 = ChangeHelper2.service.getChangeActivities(ecn);
        while(qr1.hasMoreElements()){
        	WTChangeActivity2 a = (WTChangeActivity2) qr1.nextElement();
        	if(a.getName().contains(((ChangeOrder2)obj).getNumber())) continue;
        	QueryResult qr = ChangeHelper2.service.getChangeablesAfter(a);
        	while (qr.hasMoreElements())
        		try {
        			Persistable persistable = (Persistable)qr.nextElement();
    	  ...
    	  while(qr.hasMoreElements()){
    		  ...
    		  if(p instanceof LifeCycleManaged){
    			  State stateDist = State.toState(ECNState);
    			  LifeCycleHelper.service.setLifeCycleState((LifeCycleManaged)p, stateDist);
    		  }
    	  }
      }
    }
    ...
    try {
      if ((obj instanceof WTChangeOrder2)) {
        ...
        while (qrs.hasMoreElements()) {
          ...
          QueryResult qr = ChangeHelper2.service.getChangeablesAfter(ca, true);
          while (qr.hasMoreElements()) {
            Persistable persistable = (Persistable)qr.nextElement();
  ...
  public static void setZJDECNStatus(WTObject pbo) throws MaturityException, WTException {
    ...
    throws MaturityException, WTException, WTPropertyVetoException
  {
    ...
    for (int i = 0; i < vec.size(); i++) {
      ...
      if (((!(obj instanceof EPMDocument)) && (!(obj instanceof WTPart))) || 
        (!"RELEASED_S".equals(cStatus))) continue;
  ...
  public static EPMDocument getLatestEpmItem(EPMDocument epm) {
    ...
    try {
      ...
      QueryResult qr = VersionControlHelper.service.allIterationsOf(master);
      if (qr.hasMoreElements()) {
        EPMDocument localEPMDocument = (EPMDocument)qr.nextElement();
  ...
  public static WTPart getLatestPartItem(WTPart part) {
    ...
    try {
      ...
      QueryResult qr = VersionControlHelper.service.allIterationsOf(master);
      if (qr.hasMoreElements()) {
        WTPart localWTPart = (WTPart)qr.nextElement();
        return localWTPart;
      }
    } catch (Exception e) {
      e.printStackTrace();
