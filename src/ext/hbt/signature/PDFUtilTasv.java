...
public class PDFUtilTasv {
	...
	public static String getFinalObjType(PdfReader reader,int pageNo,String topType,WTObject obj)
	{
		...
		if(isepm)
		{
			String drawingtype=PDFUtilTasv.getDrawingSize(height,width);
	...
	public static void writeToPage(String lifeCycleState,PdfStamper stamp, HashMap cooMap, HashMap inforHm, int pageNo, boolean islastpage) throws WTException
	{
		...
		while (iter.hasNext())
		{
			...
			System.out.println("   writeToPage attr name===" + attrName);
			if("RELEASED_C".equals(lifeCycleState)){
			...
			if("RELEASED_S".equals(lifeCycleState)){
				if(attrName.equals("S")||attrName.equals("C")){
					CoordinateItem coorditem = (CoordinateItem) cooMap.get(attrName);
					String value = (String) inforHm.get(attrName);
					logger.debug("   writeToPage attr value===" + value);
					if (value == null || value.trim().length() == 0)
			...
			if("RELEASED_D".equals(lifeCycleState)){
				if(attrName.equals("C")||attrName.equals("D")||attrName.equals("S")){
					CoordinateItem coorditem = (CoordinateItem) cooMap.get(attrName);
					String value = (String) inforHm.get(attrName);
					logger.debug("   writeToPage attr value===" + value);
					if (value == null || value.trim().length() == 0)
